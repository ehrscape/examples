...
// extract a postMessage appropriate origin from a URL
    ...
    window.SMART_CONNECT_HOST = function () {
        ...
        sc.on_app_launch_begin = function (a, callback) {
            callback();
        };
        sc.on_app_launch_complete = function (a, callback) {
            callback();
        };

        sc.on_app_launch_delegated_begin = function (a, callback) {
            callback();
        };
        sc.on_app_launch_delegated_complete = function (a, callback) {
            callback();
        };
        ...
        if (window.addEventListener) {
            window.addEventListener('message', procureChannel, false);
        }
        else if (window.attachEvent) {
            window.attachEvent('onmessage', procureChannel);
        }
