...
(function () {
        ...
        SMART.get_demographics()
            .success(function (demos) {
                var demographics = demos.graph
                    ...
                    .optional('?a sp:gestationalAgeAtBirth ?ga')
                    .get(0);

                var data = {name: demographics.firstname.value.toString() + " " + demographics.lastname.value.toString(),
                    gender: demographics.gender.value.toString(),
                    birthday: demographics.birthday.value.toString()};

                var res;
                if (demographics.ga) {
                    res = demos.graph.where(demographics.ga.toString() + ' sp:value ?gestationalage');
                    if (res.length === 1) {
                        data.gestationalAge = Number(res[0].gestationalage.value);
                    }
                }
        ...
        SMART.get_lab_results()
            .success(function (labs) {
                ...
                labs.graph
                    ...
                    .where('?vu sp:unit \"mo\"')
                    .each(function () {
                        data.push(new SmartBoneage(
                            this.date.value,
                            Number(this.boneage.value)
                        ));
                    });
        ...
        SMART.get_vital_sign_sets()
            .success(function (vital_signs) {
                ...
                vital_signs.graph
                    .prefix('dcterms', 'http://purl.org/dc/terms/')
                    .prefix('sp', 'http://smartplatforms.org/terms#')
                    .where('?v dcterms:date ?date')
                    .where('?v sp:height ?d')
                    .where('?d sp:value ?val')
                    .where('?d sp:unit \"cm\"')
                    .where('?d sp:vitalName ?vn')
                    .where('?vn sp:code ?code')
                    .where('?code dcterms:identifier \"8302-2\"')
                    .each(function () {
                        vitals.statureData.push({
                            date: this.date.value,
                            value: Number(this.val.value)
                        });
                    });
                ...
                vital_signs.graph
                    ...
                    .where('?d sp:vitalName ?vn')
                    .where('?vn sp:code ?code')
                    .where('?code dcterms:identifier \"8306-3\"')
                    .each(function () {
                        vitals.lengthData.push({
                            date: this.date.value,
                            value: Number(this.val.value)
                        });
                    });
                ...
                vital_signs.graph
                    ...
                    .prefix('sp', 'http://smartplatforms.org/terms#')
                    .where('?v dcterms:date ?date')
                    .where('?v sp:headCircumference  ?d')
                    .where('?d sp:value ?val')
                    .where('?d sp:unit \"cm\"')
                    .each(function () {
                        vitals.headCData.push({
                            date: this.date.value,
                            value: Number(this.val.value)
                        });
                    });
                ...
                vital_signs.graph
                    ...
                    .prefix('sp', 'http://smartplatforms.org/terms#')
                    .where('?v dcterms:date ?date')
                    .where('?v sp:bodyMassIndex  ?d')
                    .where('?d sp:value ?val')
                    .where('?d sp:unit \"kg/m2\"')
                    .each(function () {
                        vitals.BMIData.push({
                            date: this.date.value,
                            value: Number(this.val.value)
    ...
    GC.get_custom_data = function () {
        var dfd = $.Deferred();
        GC.PatientData = new GC.Model({
            vitals: {
                "length": [],
                "stature": [],
                "weight": [],
    ...
    GC.Patient = function (demographics, vitals, allergies, familyHistory, annotations, boneAge) {
     ...
     * @param familyHistory
     */
    GC.Patient.prototype.init = function (demographics, vitals, allergies, familyHistory, annotations, boneAge) {
        var map = {
    ...
    GC.Patient.prototype.getMidParentalHeight = function (forAgemos) {
        ...
        if (!this.familyHistory.father.height || !this.familyHistory.mother.height || !this.familyHistory.father.isBio || !this.familyHistory.mother.isBio) {
            return null;
        }
