            ...
            SMART.ready(function () {
                ...
                function createCapabilityAnalyzer(propName) {
                    return function (i, name) {
                        if (name == "GET") {
                            capabilities[propName].read = true;
                        } else if (name == "PUT") {
                            capabilities[propName].write = true;
                        } else if (name == "DELETE") {
                            capabilities[propName].unset = true;
                        }
                    };
                }
                ...
                GC.getContainerManifest().done(
                    ...
                    function initModelProxies(response) {//GC.deleteScratchpad()
                        if (capabilities.preferences.read && capabilities.preferences.write) {
                            debugLog("Enabling SMART preferences");
                            GC.Preferences.proxy = new GC.SmartSettingsProxy();
                        } else {
                            debugLog("SMART preferences - read or write NOT supported!\nSwitching to local storage.");
                            GC.Preferences.proxy = new GC.LocalStorageProxy("preferences");
                        }

                        if (capabilities.scratchpad.read && capabilities.scratchpad.write) {
                            debugLog("Enabling SMART scratchpad");
                            GC.Scratchpad.proxy = new GC.SmartScratchpadProxy();
                            GC._isPatientDataEditable = true;
                        } else {
                            debugLog("SMART scratchpad - read or write NOT supported!\nSwitching to local storage.");
                ...
                $("window").trigger("smartfail");

                GC.App.dialog("select-patient.html", [
                    function () {
                        GC.Preferences.proxy = new GC.LocalStorageProxy("preferences");
                        GC.Scratchpad.proxy = new GC.LocalStorageProxy("scratchpad");
                        onModelsReady();
                        GC.currentPatient = PATIENT = GC.samplePatient;
                        GC.translateFentonDatasets(PATIENT);
                        done();
                    }], {
                    "title": NS.str("STR_dlg_title_SelectDemoPatient"),
                    "height": "auto",
                    "width": 500,
                    "position": "center",
                    closeOnEscape: false
